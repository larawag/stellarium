...
QString radToDdmPStr(double angle, const int precision, const bool useD, const char flag)
{
	...
	switch (flag)
	{
		...
		case 'V': suffix = (sign?'N':'S'); widthOfDegree = 2; break; // signed N/S
	}
	...
	QTextStream os(&str);
	os << prefix ;
	os << fixed << qSetFieldWidth(widthOfDegree) << qSetPadChar('0') << d << qSetFieldWidth(0) << degsign;

	int widthOfMin = 2;
	if (precision>0)
		widthOfMin = 3 + precision;
	os << qSetRealNumberPrecision(precision);
	os << fixed << qSetFieldWidth(widthOfMin) << qSetPadChar('0') << m << qSetFieldWidth(0) << "\' " << suffix;
